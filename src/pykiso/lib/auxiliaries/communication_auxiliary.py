...
class _collect_messages(ContextDecorator):
    ...
    def __enter__(self):
        ...
        log.kiso_debug("Start queueing received messages.")
        self.com_aux.queueing_event.set()

    def __exit__(self, *exc):
        """Clear queue event to stop messages collection."""
        log.kiso_debug("Stop queueing received messages.")
...
class CommunicationAuxiliary(DTAuxiliaryInterface):
    ...
    def receive_message(
        ...
        log.kiso_debug(f"retrieved message '{response}' in {self}")
    ...
    def clear_buffer(self) -> None:
        ...
        log.kiso_info("Clearing buffer. Previous responses will be deleted.")
    ...
    def _run_command(self, cmd_message: str, cmd_data: bytes = None) -> bool:
        ...
        if cmd_message == "send":
            ...
            log.kiso_debug(f"ignored command '{cmd_message} in {self}'")
        else:
            log.kiso_warning(f"received unknown command '{cmd_message} in {self}'")
    ...
    def _receive_message(self, timeout_in_s: float) -> None:
        ...
        try:
            ...
            if msg is not None and self.queueing_event.is_set():
                log.kiso_debug(f"received message '{rcv_data}' from {self.channel}")
