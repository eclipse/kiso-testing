...
class UdsCallback:
    ...
    def __post_init__(self):
        ...
        if self.response_length is None:
            self.response_length = self.response
        else:
...
class UdsServerAuxiliary(AuxiliaryInterface):
    ...
    def register_callback(self, request, response, response_length: Optional[int] = None):
    ...
    def _dispatch_callback(self, received_uds_data: bytes):
        if self.uds_config_enable:
            ...
            did_to_name = self.sid_to_did_to_name[received_uds_data[0]]
            name = did_to_name.get(received_uds_data[1]) or did_to_name.get(received_uds_data[1:3])
            if name is None:
                return
            # run the positive response function registered for the "human name"
            service_name = get_uds_service(received_uds_data[0])
            ...
            ### ALTERNATIVE
            service_encode_func = service_to_positive_response_func[received_uds_data[0]]
        else:
        ...
        self.uds_config.tp.encode_isotp(
