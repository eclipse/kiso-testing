...
class InstrumentControlAuxiliary(DTAuxiliaryInterface):
    ...
    def write(
        ...
        :param validation: contain validation criteria apply on the
            response
        """
        log.kiso_debug(f"Sending a write request in {self} for {write_command}")
    ...
    def read(self) -> Union[str, bool]:
        ...
        :return: received response from instrument otherwise empty
            string
        """
        log.kiso_debug(f"Sending a read request in {self}")
    ...
    def query(self, query_command: str) -> Union[bytes, str]:
        ...
        :return: Response message, None if the request expired with a
            timeout.
        """
        log.kiso_debug(f"Sending a query request in {self}) for {query_command}")
    ...
    def _create_auxiliary_instance(self) -> bool:
        ...
        try:
            # Open instrument
            log.kiso_info("Open instrument")
            self.channel.open()

            # Enable remote control
            log.kiso_info("Enable remote control")
            ...
            if self.output_channel is not None:
                log.kiso_info(f"Select channel {self.output_channel}")
                command, validation = self.helpers.get_command(
                ...
                log.kiso_info("Using default output channel.")
        except Exception:
