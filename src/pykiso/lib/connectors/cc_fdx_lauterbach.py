...
class CCFdxLauterbach(connector.CChannel):
    ...
    def load_script(self, script_path: str):
        ...
        if err != 0:
            ...
            return err
        log.kiso_info(f"Loading the cmm file: {script_path}")
    ...
    def _cc_open(self) -> bool:
        ...
        try:
            ...
            log.kiso_debug("Trace32 remote API loaded")
        ...
        if self.t32_api.T32_Init() == 0:
            ...
            log.kiso_debug("Previous process Trace32 closed")
        ...
        if self.t32_api.T32_Ping() == 0:
            log.kiso_debug(f"ITF connected on {self.node}:{self.port}")
        else:
        ...
        if self.load_script(self.t32_fdx_clr_buf_script_path) == 0:
            log.kiso_info(f"script {self.t32_fdx_clr_buf_script_path} loaded")

        # Load the cmm script
        if self.load_script(self.t32_main_script_path) == 0:
            log.kiso_debug(f"script {self.t32_main_script_path} loaded")
        else:
    ...
    def _cc_close(self) -> None:
        ...
        log.kiso_debug(f"Disconnected from FDX {self.fdxin} with state {fdxin_state}")

        # Close FDX sender communication
        fdxout_state = self.t32_api.T32_Fdx_Close(self.fdxout)
        log.kiso_debug(f"Disconnected from FDX {self.fdxout} with state {fdxout_state}")

        # Reset Target
        reset_cpu_state = self.t32_api.T32_ResetCPU()
        log.kiso_debug(f"Reset the CPU with state {reset_cpu_state}")
    ...
    def _cc_send(self, msg: Message or bytes, raw: bool = False) -> int:
        ...
        :return: poll length
        """
        log.kiso_debug(f"===> {msg}")
        log.kiso_debug(f"Sent on channel {self.fdxout}")
    ...
    def _cc_receive(
        ...
        while not is_timeout:
            ...
            # Check if a message has been received
                ...
                log.kiso_info(f"Message size: {poll_len}")
                log.kiso_info(
                    f"<=== {Message.parse_packet(buffer.contents.raw[:poll_len])}"
                )
                log.kiso_debug(f"Received on channel {self.fdxin}")
    ...
    def reset_board(self) -> None:
        ...
        log.kiso_debug(f"In Reset Function safety flag is: {self.safe_reset_flag}")
        log.kiso_debug(f"In Reset Function reset flag is: {self.reset_flag}")

        while not self.safe_reset_flag:
            log.kiso_debug(f"Safety flag in while loop is: {self.safe_reset_flag}")
            time.sleep(0.3)

        self.reset_flag = True
        log.kiso_debug("Do the board reset")
