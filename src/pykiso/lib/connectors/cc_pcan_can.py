...
class TRCWriterCanFD(TRCWriter):
    ...
    FORMAT_MESSAGE_V2_0 = (
        "{msgnr:>7} {time:13.3f} DT {id:>8} {dir:>2} -  {dlc:<4} {data}"
    )


    def _write_header_V2_0(self, start_time: datetime) -> None:
    ...
    def write_header(self, timestamp: float) -> None:
        ...
        if self.file_version == TRCFileVersion.V1_0:
            ...
            self._write_header_V2_0(start_time)
        elif self.file_version == TRCFileVersion.V2_1:
...
class CCPCanCan(CChannel):
    ...
    def _merge_trc(self) -> None:
        ...
        try:
            ...
            with can.TRCReader(list_of_traces[0]) as reader:
                messages = list(reader)
                start_time = reader.start_time
                file_version = reader.file_version
                log.error(f"START TIME IS : {start_time}")

            # Only keep first trace header
            with list_of_traces[0].open("r") as trc:
                data = trc.read().splitlines(True)

            with list_of_traces[0].open("w") as trc:
                for line in data:
                    if line[0] == ";":
                        trc.write(line)
      
            first_time_stamp = 0
            # Correct offset and append messages from all traces
            for trc in list_of_traces:
                with can.TRCReader(trc) as reader:
                    current_trc_messages = list(reader)
                    current_trc_start_time = reader.start_time
                    # Offset in miliseconds
                    offset = (current_trc_start_time - start_time).total_seconds() * 1000
                    log.error(f"The offset is: {offset}")
                    
                    #current_trc_messages = CCPCanCan.remove_offset(current_trc_messages, offset)
                    messages = current_trc_messages
            
            
                with TRCWriterCanFD(result_trace) as writer:
                    #TODO: Detect
                    writer.file_version = TRCFileVersion.V2_0
                    for msg in messages:
                        if first_time_stamp == 0:
                            first_time_stamp = msg.timestamp
                        time = msg.timestamp - first_time_stamp
                        time = time + offset
                        writer.on_message_received(msg)

                if trc is not result_trace:
                    #os.remove(trc)
                    pass
    ...
    @staticmethod
    def remove_offset(messages: List, offset) -> List:
        for msg in messages:
