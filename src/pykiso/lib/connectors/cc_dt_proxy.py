...
"""
Double Threaded Proxy Channel
*****************************

:module: cc_dt_proxy

:synopsis: CChannel implementation for multi-auxiliary usage
    (only usable with DTProxyAuxiliary).
...
class CCDTProxy(CChannel):
    ...
    def detached_tx_callback(self, func: Callable) -> None:
        with self._lock:
            log.warning(f"reset current attached transmit callback {func.__name__}")
            self._tx_callback = None
