...
class ModuleCache:
    ...
    * active instance (lazily created)
    ...
    def provide(self, name: str, module: str, **config_params):
        ...
        :param module: eihter 'python-file-path:Class' or 'module:Class' of the class we want
            to provide
        """
    ...
    def _import(self, name: str):
        ...
        if ".py" in location:
            ...
            if path_loc.exists() and path_loc.is_file():
                ...
                logging.debug(f"loading {_class} as {name} from {path_loc}")
            else:
                raise ImportError(
        ...
        logging.debug(f"loaded {_class} as {name} from {location}")
        return cls

    def get_instance(self, name: str):
        """get an instace of alias <name> (create and configure one of not existend)"""
        if name in self.instances:
            logging.debug(f"instance for {name} found ({self.instances[name]})")
            return self.instances[name]
        if name not in self.modules:
            logging.debug(f"module for {name} not found, loading...")
        ...
        logging.debug(f"instantiated {name}")
        return inst
...
class AuxiliaryCache(ModuleCache):
    ...
    def __init__(self, con_cache: ModuleCache):
        super(AuxiliaryCache, self).__init__()
    ...
    def provide(self, name: str, module: str, connectors=None, **config_params):
        ...
        super(AuxiliaryCache, self).provide(name, module, **config_params)
    ...
    def get_instance(self, name: str):
        ...
        inst = super(AuxiliaryCache, self).get_instance(name)
        if not inst.is_instance:
            inst.create_instance()
            logging.debug(f"called create_instance on {name}")
        self.instances[name] = inst
        return inst
    ...
    def _stop_auxiliaries(self):
        ...
        for aux in self.instances.values():
            logging.debug(f"issuing stop for auxiliary '{aux}'")
            aux.stop()
...
class DynamicImportLinker:
    ...
    def install(self):
        ...
        logging.debug(f"installed the {self.__class__.__name__}")
    ...
    def provide_connector(self, name: str, module: str, **config_params):
        ...
        :param module: eihter 'python-file-path:Class' or 'module:Class' of the class we want
            to provide
        """
        logging.debug(f"provided connector {name} (at {module})")
    ...
    def provide_auxiliary(self, name: str, module: str, aux_cons=None, **config_params):
        ...
        """
        logging.debug(f"provided auxiliary {name} (at {module})")
    ...
    def uninstall(self):
        ...
        logging.debug("closing and uninstalling all dynamic modules and loaders")
